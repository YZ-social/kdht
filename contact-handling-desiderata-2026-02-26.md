...
The argument for limiting: while each individual forwarding chain is bounded, if many nodes simultaneously request forwarding through the same popular node, the aggregate load on that node could crowd out its normal RPC processing. Now that the H2 timeout bug is fixed, each chain also has a time bound (~2.5s), which may make explicit concurrency limiting unnecessary.
