...
Each published datum is individually expired at the `issuedTime + expiration`, and removed from the network. Data may be republished by publishing to the same `key` and `subject`, or cancelled by doing the same with a `null payload`. Subscribed nodes do have the `handler` called with the updated or explicitloy cancelled payloads, but not for expired data. A newly subscribed node receives previously published data that has not expired or been cancelled.

The `payload` may also be ommitted, which does not change any existing payload data, but extends the expiration time based on the other parameters, with the limitation that the new expiration is never earlier than has been the case before.
